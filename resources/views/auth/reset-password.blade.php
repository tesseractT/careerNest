                        ...
                        <form class="login-register text-start mt-20" method="POST" action="{{ route('password.store') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" id="input-1"
                                    type="email" required="" name="email" value="{{ old('email', $request->email) }}"
                                    placeholder="stevenjob@gmail.com">
                                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>

                            <div class="form-group">
                                <label class="form-label" for="input-2">Password *</label>
                                <input class="form-control {{ $errors->has('password') ? 'is-invalid' : '' }}"
                                    id="input-2" type="password" required="" name="password" placeholder="********">
                                <x-input-error :messages="$errors->get('password')" class="mt-2" />
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="input-3">Confirm Password *</label>
